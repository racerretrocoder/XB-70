var reheat = func {
  ...
  if (IAS > 630) {
    spoilers = spoilers+(IAS-630)/100;
  }
  if ((mach > 0.9) and (mach <= 1.0)) {
    spoilers = spoilers+0.20*(mach-0.9)/0.1;
  }
  if ((mach > 1.0) and (mach < 1.15)) {
    spoilers = spoilers+0.20*(1.15-mach)/0.15;
  }
  ...
  # ********************** Wing sweep automaticly
  if ((mach > 0.95) or (IAS > 400)) {
    fold = 0.4;
  }
  if (mach > 1.4) {
    fold = 1.0;
